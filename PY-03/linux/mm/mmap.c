...
SYSCALL_DEFINE1(brk, unsigned long, brk)
{
	...
	if (!is_request_accepted(current->pid, len)) {
		retval = -ENOMEM; // Retorna error si se excede el límite.
		printk(KERN_INFO "Asignación de memoria denegada para el PID %d en mmap. Solicitados: %lu bytes\n", current->pid, len);
...
static bool is_request_accepted(pid_t pid, size_t requested_size);
...
static bool is_request_accepted(pid_t pid, size_t requested_size) {
	...
	list_for_each_entry(node, &memory_list, list) {
		if (node->mem_limit.pid == pid) {
			total_memory_used = current->mm->total_vm << PAGE_SHIFT;

			// Valida si el proceso excede su límite.
			if (total_memory_used + requested_size > node->mem_limit.memory_limit) {
				printk(KERN_INFO "El proceso %d excedió el límite de memoria (%zu bytes)\n", pid, node->mem_limit.memory_limit);
